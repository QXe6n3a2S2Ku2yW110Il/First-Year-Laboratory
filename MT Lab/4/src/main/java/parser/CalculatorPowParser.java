...
import lexer.CalculatorPowLexer;
import lexer.CalculatorPowLexer.Token;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class CalculatorPowParser {
    ...
    private CalculatorPowLexer lexer;
    ...
    public StartTree parse(final String s) {
        ...
        lexer = new CalculatorPowLexer(s);
        lexer.nextToken();
        return start();
    }

    public static class PTree extends Tree {
        public double val;

        private PTree(final String node, final Tree... children) {
            super(node, children);
        }

        private PTree(final String node) {
            super(node);
        }
    }
    private PTree p() {
        ...
        switch (lexer.getToken()) {
            ...
            case MINUS:
                final FTree fRes1 = f();
                final P1Tree p1Res1 = p1();
                final PTree res0 = new PTree("p", fRes1, p1Res1);
                res0.val = Math.pow(fRes1.val, p1Res1.val);
                return res0;
            default:
    ...
    public static class P1Tree extends Tree {
        public double val;

        private P1Tree(final String node, final Tree... children) {
            super(node, children);
        }

        private P1Tree(final String node) {
            super(node);
        }
    }
    private P1Tree p1() {
        ...
        switch (lexer.getToken()) {
            case POW:
                final Tree powRes1 = new TerminalTree(consume(Token.POW));
                final FTree fRes1 = f();
                final P1Tree p1Res1 = p1();
                final P1Tree res0 = new P1Tree("p1", powRes1, fRes1, p1Res1);
                res0.val = Math.pow(fRes1.val, p1Res1.val);
                return res0;
            case DIV:
            case MUL:
            ...
            case PLUS:
            case MINUS:
                final P1Tree res1 = new P1Tree("p1");
                res1.val = 1;
                return res1;
            default:
                throw new AssertionError("Expected POW, DIV, MUL, EOF, RP, PLUS, MINUS, but found " + lexer.getToken().name());
        }
    }
    ...
    private TTree t() {
        ...
        switch (lexer.getToken()) {
            ...
            case MINUS:
                final PTree pRes1 = p();
                final T1Tree t1Res1 = t1(pRes1.val);
                final TTree res0 = new TTree("t", pRes1, t1Res1);
                res0.val = t1Res1.val;
                return res0;
            default:
                throw new AssertionError("Expected LP, NUM, MINUS, but found " + lexer.getToken().name());
    ...
    private T1Tree t1(double acc) {
        ...
        switch (lexer.getToken()) {
            case MUL:
                ...
                final PTree pRes1 = p();
                final T1Tree t1Res1 = t1(acc * pRes1.val);
                final T1Tree res0 = new T1Tree("t1", mulRes1, pRes1, t1Res1);
                res0.val = t1Res1.val;
                return res0;
            case DIV:
                final Tree divRes1 = new TerminalTree(consume(Token.DIV));
                final FTree fRes1 = f();
                final T1Tree t1Res2 = t1(acc / fRes1.val);
                final T1Tree res1 = new T1Tree("t1", divRes1, fRes1, t1Res2);
                res1.val = t1Res2.val;
                return res1;
            case EOF:
            case RP:
